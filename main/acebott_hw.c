...
void acebott_init(void) {
    ...
    // Outputs: Motors, Trig, Headlights, Buzzer
    gpio_config_t out_conf = {
        .pin_bit_mask = (1ULL << PIN_MOTOR_EN) | (1ULL << PIN_MOTOR_DATA) |
                        (1ULL << PIN_MOTOR_LATCH) | (1ULL << PIN_MOTOR_CLOCK) |
                        (1ULL << PIN_ULTRASONIC_TRIG) | (1ULL << HEADLIGHT_L_GPIO) |
                        (1ULL << HEADLIGHT_R_GPIO) | (1ULL << BUZZER_GPIO),
...
float acebott_get_distance(void) {
    ...
    while (gpio_get_level(PIN_ULTRASONIC_ECHO) == 0) {
        if (esp_timer_get_time() - start_wait > 20000) return -1.0f;
    }
    uint64_t t1 = esp_timer_get_time();
    while (gpio_get_level(PIN_ULTRASONIC_ECHO) == 1) {
        if (esp_timer_get_time() - t1 > 25000) return -1.0f;
    }
    uint64_t t2 = esp_timer_get_time();
    return (float)(t2 - t1) / 58.0f;
}

void acebott_read_line_sensors(uint32_t *l, uint32_t *m, uint32_t *r) {
    if(l) *l = adc1_get_raw(ADC1_CHANNEL_7);
    if(m) *m = adc1_get_raw(ADC1_CHANNEL_0);
    if(r) *r = adc1_get_raw(ADC1_CHANNEL_3);
}
...
void acebott_flash_headlights(int count, uint32_t duration_ms) {
    for (int i = 0; i < count; i++) {
        ...
        if (i < count - 1) vTaskDelay(pdMS_TO_TICKS(duration_ms));
    }
}
    ...
    if (items) {
        ...
        if (num_items >= 34) {
            for (int i = 1; i < 33 && i < num_items; i++) {
                code <<= 1;
                if (items[i].duration1 > 1000) code |= 1;
            }
        }
        vRingbufferReturnItem(rb, (void*)items);
